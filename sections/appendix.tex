...
We let $S=k[x_1, \dots, x_n]$ be a polynomial ring graded by an arbitrary finitely generated abelian group $\Cl(S)$ via a map $\ZZ^n\overset{\deg}{\longrightarrow}\Cl(S)$ where $\epsilon_1, \dots, \epsilon_n$ are a fixed basis of $\ZZ^n$ and where we set $\deg(x_i) \ce\deg(\epsilon_i)\in \Cl(S)$ and $\deg(a)=0$ for $a\in k$.  \lauren{I don't see why this is implied otherwise}  To avoid trivialities, we will always assume that the map $\deg$ is surjective.
...
\begin{proof}
  ...
  \begin{prop}
    The following are equivalent: \lauren{the logic of this reads very strangely to me currently .. there does indeed exist a polynomial ring satisfying the conditions in (1) so it seems like (2) should always be true}
    \begin{enumerate}
      ...
      \begin{enumerate}
        ...
        \item $S$ is positively graded;
        \item Each row of $\alpha$ \lauren{what is $\alpha$?} is the primitive point on a distinct ray; that is, the rows of
        $\alpha$ are distinct, nonzero, and have greatest common divisor 1. \lauren{what does it mean for vectors to have greatest common divisor 1?}
  ...
  \begin{proof}
    ...
    (1) implies (2) is more subtle.  The $\Cl(S)$-grading on $S$ induces a GIT problem and hence a secondary fan. \lauren{do we really know this for all gradings?  if not what are we using?} If $X_{\Sigma}$ is the toric variety associated to some \lauren{chamber?} of the secondary fan, Proposition 14.4.1 \de{14.4.1 or 14.3.2?} \lauren{not sure, 14.3.2 is the basic fact about Gale duality stated below but I think we may need more!} (a basic fact on Gale duality) implies that the support of the fan $\Sigma$ is the cone spanned by the rays in $N_{\QQ}$.
