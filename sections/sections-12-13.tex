...
\begin{example}\label{ex:BlowupAtOnePoint}
    Let $S$ be the Cox ring of $\cH_1$ the Hirzebruch surface of type $1$.  In this case, the secondary fan as two chambers: one chamber corresponds to $\PP^2$ and one corresponding to $\cH_1$. There is a natural morphism $\alpha\colon \cH_1\to \PP^2$ given by the blowup of $\PP^2$ at point, and thus we can take $\widetilde{X} = \cH_1$, $\pi_1 = \alpha \co \cH_1 \to \PP^2$, and $\pi_2 = \id \co \cH_1 \to \cH_1$.  We thus have
    $
...
\]
At a more concrete level, the Fourier--Mukai transform can be understand as follows.  There is a Hanlon-Hicks-Lazarev complex $\cR$ that resolves $\Gamma_{ij}\subseteq \sX_i\times \sX_j$ and so we can ``underive'' the tensor product, by replacing $\cO_{\Gamma_{ij}}$ by $\cR$:
\[
...
\begin{example}
    ...
    \]
We thus have that $\Phi_{ij}(\cO_{\sX_j})$ is the pushforward of $\cO_{\Gamma_{ij}}$ under the natural birational morphism $\Gamma_{ij}\to \sX_j$.  We then have $R(q|_{\Gamma_{ij}})_* \cO_{\Gamma_{ij}} = \cO_{\sX_j}$ by \cite{BBB+24}*{Proposition 3.5}.
\end{example}
...
\begin{equation}
\label{eqn:hyperss}
E^{p,q}_1 = R^tq_*(\cC^s) \Rightarrow R^{s + t}q_*(\cC). 
\end{equation}
