...
As we saw in Theorem~\ref{thm:beilinsonFSEC}, the
\defi{Beilinson collection} of line bundles $\cO_{\PP^n}(-n), \ldots,
\cO_{\PP^n}(-1), \cO_{\PP^n}$ is a full strong exceptional collection for $D^b(\PP^n)$. This result extends to products of projective spaces.  

\begin{example}
    The Beilinson collection for $\PP^1$ consists of the line bundles of degrees $-1$ and $0$.   On $\PP^1\times \PP^1$, the bundles $\cO_{\PP^1\times \PP^1}(-1,-1),\cO_{\PP^1\times \PP^1}(-1,0), \cO_{\PP^1\times \PP^1}(0,-1),\cO_{\PP^1\times \PP^1}$ form a full, strong exceptional collection.  Notice that the line bundles that arise are the box products of the bundles in the Beilinson collections of the factors. A similar result holds for any product $\PP^n\times \PP^m$.  In this case, the line bundles that appear are $\cO(-i,-j)$ for $-n \leq -i \leq 0$ and $-m \leq -j \leq 0$.  In fact, this extends to any product~$\PP^{n_1} \times \cdots \times \PP^{n_t}$. The proof follows easily from Beilinson's original work.  This is for the following reasons:
    \begin{enumerate}
        \item Exceptionality is a simple sheaf cohomology calculation.
    ...
    \end{enumerate}
    For instance, let us consider the case of $\PP^1 \times \PP^1$. The fact that
    \[
    \cO_{\PP^1\times \PP^1}(-1,-1),\cO_{\PP^1\times \PP^1}(-1,0), \cO_{\PP^1\times \PP^1}(0,-1),\cO_{\PP^1\times \PP^1}
    \]form a strong exceptional collection is a simple cohomology computation, using the K\"unneth formula.  For fullness, we start with the fact that
    \[
    \cO_{\PP^1 \times \PP^1} \gets \cO_{\PP^1 \times \PP^1}(-1,-1)
    \]
     is a resolution of $\Delta_{\PP^1} \subseteq \PP^1 \times \PP^1$.  Taking the box product of this complex with itself yields a resolution of $\Delta_{\PP^1 \times \PP^1} \subseteq (\PP^1)^4$:
     \[
    \cO_{(\PP^1)^4} \gets
    \begin{matrix}
      \cO_{(\PP^1)^4}(-1,0,-1,0)\\
    ...
    \gets
     \cO_{(\PP^1)^4}(-1,-1,-1,-1) .
     \]
Applying Construction~\ref{const:monad} gives fullness, just as in the proof of Theorem~\ref{thm:beilinsonFSEC}.
\end{example}



Some other classical examples of smooth projective toric varieties also satisfy an analogue of Beilinson's theorem.
\begin{example}
If $X$ is a Hirzebruch surface, then
$$
\cO_{X}(-1,-1),\cO_{X}(-1,0), \cO_{X}(0,-1),\cO_{X}
$$
is a full, strong exceptional collection for $D^b(X)$.   For those who are curious: this is a special case of Orlov's semiorthogonal decomposition result for projective bundles \cite[Cor.~8.36]{Huybrechts2006}.
\end{example}

This raises a natural question:  which smooth, projective toric varieties satisfy an analogue of Beilinson's theorem?  In 1997, King conjectured that the answer was ``all of them''.
...
\begin{remark}
\label{rem:monad}
A variant of Corollary~\ref{cor:UniquenessAndFSEC} holds for any variety satisfying King's Conjecture. 
\end{remark} 

King's Conjecture turned out to be
false.  Hille-Perling gave the first counterexamples, and followup work of Michalek and Efimov gave counterexamples for fairly simple varieties and showed that the conjecture cannot be salvaged by restricting, for instance, to Fano toric varieties
~\cites{HP06, Michalek11a, Efimov14}.
...
~\cites{ABK+20,
BFK19,BDL+20,BIL+26,BDM22,BT09,BH09,BO19,BW21,CM04,CM10,DLM09,Jerby15,Jerby17,Jerby20,Kawamata06,Kawamata13,LM11,OU13,PrabhuNaik17,Sanchez23,Uehara14},
\emph{inter alia}.  The closest result to King's Conjecture was probably Kawamata's theorem, which proved a weakening of the conjecture by dropping the line bundle requirement and removing the strongness condition~\cite{Kawamata06}.  While this is a major result, the contrast with Beilinson's theorem is pretty stark: while Corollary~\ref{cor:UniquenessAndFSEC} yields unique representations of any sheaf in terms of well-known building blocks, namely the line bundles in the Beilinson collection, the representations from Kawamata's would fail to be unique, and they would be in terms of building blocks that we don't understand whatsoever.  Notably: it is an open question whether the objects in Kawamata's collection are sheaves or whether they will necessarily involve complexes of sheaves.
...
\begin{enumerate}
    ...
    \item  The Bondal-Thomsen collection--which is also uniform for all toric varieties with the same Cox ring--is an excellent analogue of the Beilinson collection in many ways.
\end{enumerate}
...
\end{tabular}
\end{center}
Note that the Bondal-Thomsen collection and the HHL resolution depend only on the Cox ring.  This suggests that the ``right'' analogue of Theorem~\ref{thm:beilinsonFSEC}, i.e. the ``right'' version of King's Conjecture, should perhaps depend only on the Cox ring. That is, a revised King's Conjecture should involve a category that combines the sheaves from all of the toric varieties with the same Cox ring.
...
More specifically, we have seen that a multigraded polynomial ring $S$ determines a finite set of simplicial toric varieties $X_1, X_2, \dots, X_r$ each of which has a corresponding stack $\sX_1, \sX_2, \dots, \sX_r$.
...
\end{lemma}
\begin{proof}
    At the level of varieties, this would be very standard as one could just take a simplicial refinement of the fans of each of the $X_i$.  Extending that proof to toric stacks requires working with additional notation, but there are very few new ideas and so we omit the proof.  See \cite{BBB+24}*{\S 3.1} for details.
\end{proof}

A standard fact \cite{BBB+24}*{Lemma 3.7} is that the (derived) pullback $\pi_i^*$ from $D(\sX_i)$ to $D(\widetilde{\sX})$ is fully faithful.\footnote{Because we are focused on derived categories, every time we write $\pi_i^*$ we should really be using the derived functor $L\pi_i^*$.  However, since we focus almost entirely on the pullbacks of line bundles, $L\pi_i^*$ and $\pi_i^*$ will coincide.  We will thus abuse notation and write $\pi_i^*$ throughout.}  Thus, $\pi_i^*$ embeds each derived category of $\sX_i$ into the derived category of $\widetilde{\sX}$.
...
We define the Cox category of $S$, denoted $D_{\Cox}(S)$ as follows.  Since each pullback functor $\pi_i^*$ is fully faithful, $\pi_i^* D(\sX_i)$ is a copy of $D(\sX_i)$ that lives in $D(\widetilde{\sX})$.
We define $D_{\Cox}(S)$ as the subcategory of~$D(\widetilde{\sX})$ generated\footnote{By ``generated'' we mean the smallest triangulated category containing these objects and that is closed under taking summands.} by $\pi_1^*D(\sX_1), \pi_2^* D(\sX_2), \dots, \pi_r^* D(\sX_r)$. In other words, $D_{\Cox}(S)$ is like the category generated by (the pullbacks of) all sheaves from all of the $\sX_i$.
\end{defn}
It is easy to check that the definition of $D_{\Cox}(S)$
does not depend upon the choice of $\widetilde{\sX}$ from Lemma~\ref{lem:TildeX}; see \cite{BBB+24}*{Corollary 3.9}. But these copies overlap.
\begin{example}
...
We can thus think of $D_{\Cox}(S)$ as ``gluing'' the various $D(\sX_i)$ together.  See Figure~\ref{fig:venn_diagram}.  The main results of the next several lectures show that $D_{\Cox}(S)$ has nicer properties than any of the individual $D(\sX_i)$, in a manner that is (metaphorically at least) akin to how a projective variety can have nicer properties than its various affine patches.
...
\begin{example}\label{ex:CoxProjectiveSpace}
    Let $S$ be the Cox ring of $\PP^n$.  In this case, the secondary fan only has a single chamber, i.e. the chamber corresponding to $\PP^n$ and thus $D_{\Cox}(S)=D(\PP^n)$.  The same holds for any toric variety whose secondary fan has a single chamber, such as weighted projective space or a product of projective spaces.
\end{example}

\begin{example}\label{ex:BlowupAtOnePoint}
    Let $S$ be the Cox ring of $\cH_1$ the Hirzebruch surface of type $1$.  In this case, the secondary fan has two chambers: one chamber corresponds to $\PP^2$ and one corresponding to $\cH_1$. There is a natural morphism $\alpha\colon \cH_1\to \PP^2$ given by the blowup of $\PP^2$ at point, and thus we can take $\widetilde{X} = \cH_1$, $\pi_1 = \alpha \co \cH_1 \to \PP^2$, and $\pi_2 = \id \co \cH_1 \to \cH_1$.  We thus have
    $
    D_{\Cox}(S) = \langle \pi_1^* D(\PP^2), \pi_2^* D(\cH_1)\rangle =
      D(\cH_1)
    $.
\end{example}

\begin{example}\label{ex:Hirzebruch}
    Let $S$ be the Cox ring of $\cH_3$ the Hirzebruch surface of type $3$.  In this case, the secondary fan has two chambers: one chamber corresponds to a weighted projective space $\PP(1,1,3)$ and one corresponding to $\cH_3$.  Unlike the previous example, $\PP(1,1,3)$ is simplicial but not smooth, and so we need to consider the weighted projective stack $\PP_{\operatorname{stack}}(1,1,3)$. There is a morphism from $\cH_3$ to the weighted projective space $\PP(1,1,3)$, where the pullback of the line bundle $\cO_{\PP(1,1,3)}(3)$ is $\cO_{\cH_3}(0,1)$.  But there is no such morphism to $\PP_{\operatorname{stack}}(1,1,3)$.  The reason for this is simple:  if there were such a morphism $\alpha\colon \cH_3 \to \PP_{\operatorname{stack}}(1,1,3)$, then since $\cO(1)$ is a line bundle on the stack, its pullback would need to be a line $L$ such that $L^{\otimes 3} = \alpha^* \cO(3) = \cO_{\cH_3}(0,1)$, which is impossible. This issue is avoided on the weighted projective space because $\cO(1)$ is not a line bundle on that space. Thus, we have our first example where $D_{\Cox}(S)$ is a genuine combination of two subcategories.
\end{example}



\begin{remark}\label{rmk:mythical}
    It is rarely the case that there is some $\widetilde{\sX}$ as in Lemma~\ref{lem:TildeX} where $D_{\Cox}(S)$ equals $D(\widetilde{\sX})$.  In other words, $D_{\Cox}(S)$ is generally a proper subcategory of $D(\tsX)$, as illustrated somewhat cartoonishly in Figure~\ref{fig:venn_diagram}. Nevertheless, it can sometimes be helpful to think of the Cox category as the derived category of a mythical toric variety $\sX_{\text{myth}}$ associated to the Cox ring $S$.  This $\sX_{\text{myth}}$ would be smooth and proper and birational to each of the $\sX_i$. This yields a heuristic for properties of $D_{\Cox}(S)$, and as we will see in \S\ref{subsec:smooth}, some of those resulting predictions are correct.
\end{remark}

\begin{remark}
Let us continue with the metaphor that $D_{\Cox}(S)$ is like a projective variety, obtained by gluing the ``affine patches'' $D(\sX_i)$.  One might ask:  what is the analogue of the transition functions from one affine patch to another?  The answer is the Fourier--Mukai transforms associated to the birational maps $\sX_i \dashrightarrow \sX_j$.  We will examine these transforms in greater depth in Section \ref{sec:FMTransform}.
% these copies
...
% transition function, and each $\pi_{i*}\colon D_{\Cox} \to D(\sX_i)$
% represents restriction to the patch.
One can also make this metaphor more precise, and totally excise the cover $\tsX$, by using the Grothendieck construction \cite{SGA1}*{\S\.VI.8}.  This gets pretty technical, and so we omit it in these notes; see \cite{BBB+24}*{Appendix A} for details.
...
We have defined $D_{\Cox}(S)$, but to make Theorem~\ref{thm:main1} precise, we also need to define the Bondal--Thomsen collection as a subset of $D_{\Cox}(S)$.  We  return to our running example before giving the full definition.
...
Thus, $D_{\Cox}$ is the subcategory of $D(\widetilde{\X})$ generated by pullbacks of line bundles from $\cH_3$ and $\PP(1,1,3)$.
...
The Bondal--Thomsen collection corresponds to a set of degrees in $\Cl(\cH_3)= \ZZ^2$.  Each of these degrees has the form $-d_i$ where $d_i$ is some effective degree.  The secondary fan lives in $\Cl(\cH_3)\otimes_{\ZZ} \RR$ and has support equal to the effective cone of $\cH_3$.  So we can overlay the secondary fan with the $d_i$ from the  Bondal--Thomsen collection.  This is illustrated in
...
$D_{\Cox}$. The issue is that we have too many possible choices!  For a given $-d$ should we take the pullback of $\cO_{\PP(1,1,3)}(-d)$ or the pullback of $\cO_{\cH_3}(-d)$.  If these always coincided, we would not have an issue, but we are not that lucky.
...
We thus pull back $-d_1$ and $-d_5$ from $\cH_3$, and $-d_2$ and $-d_3$ from $\PP(1,1,3)$; for $-d_0$ and $-d_4$, either choice will work because the pullbacks coincide.
\end{example}
...
Each of the toric stacks $\sX_1, \dots, \sX_r$ contains a copy of the same dense torus.  In particular, there is a natural birational map $\sX_i\dashrightarrow \sX_j$ for any $i,j$.  We write $\Gamma_{ij}$ for the graph of this birational map as a substack of $\sX_i\times \sX_j$.  There are natural projection maps $p,q$ from $\sX_i\times \sX_j$ to the two factors. We obtain the following diagram:
\[
\xymatrix{
...
\]
At a more concrete level, the Fourier--Mukai transform can be understood as follows.  There is a Hanlon-Hicks-Lazarev complex $\cR$ that resolves $\Gamma_{ij}\subseteq \sX_i\times \sX_j$ and so we can ``underive'' the tensor product, by replacing $\cO_{\Gamma_{ij}}$ by $\cR$:
\[
\Phi_{ij}(\cE) = Rq_*\left(p^*\cE \otimes_{\cO_{\sX_i\times \sX_j}} \cR\right).
\]
This provides a geometric way to ``move'' a sheaf (or an object of the derived category) from one of the $\sX_i$ to all of the others.  Namely, imagine we have a sheaf $\cE_1$ on $\sX_1$.  For each $j$, we obtain $\Phi_{1j}(\cE_1)$ on $\sX_j$.  Since the $\sX_i$ all agree on the dense torus, $\Phi_{1j}(\cE_1)$ will agree with $\cE_1$ on the dense torus and will only exhibit different behavior on the toric boundary.

We will not use the following result in this lecture, but it will be used later. 
\begin{prop}
\label{prop:pullpush}
Let $\widetilde{\X}$ be as in the definition of the Cox category, and let $\pi_i \co \widetilde{\X} \to \X_i$ be the accompanying maps for all $i$. There is an isomorphism $\Phi_{ij} \cong R\pi_{j *}\pi_i^*$.
\end{prop}

\begin{proof}[Sketch of proof]
Let $t_i \co \Gamma_{ij} \to \X_i$ and $t_j \co \Gamma_{ij} \to \X_j$ be the canonical maps. The first observation is that $\Phi_{ij}$ is given by $Rt_{j*} t_i^*$; this follows from the projection formula, in a manner similar to the proof of Proposition~\ref{prop:diagonalidentity}. Now, the claim is that there exists a map $\pi \co \widetilde{X} \to \Gamma_{ij}$ such that $\pi^*$ is fully faithful, $t_i\pi = \pi_i$, and $t_j \pi = \pi_j$. Indeed, the fully faithfulness of $\pi^*$ is equivalent to the unit of adjunction $\id \to R\pi_*\pi^*$ being an isomorphism, and so we have $\Phi_{ij} = Rt_{j*} t_i^* = R(t_{j} \circ \pi)_*(t_i\pi)^* = R\pi_{j*} \pi_i^*$. The map $\pi$ may be induced by a refinement of stacky fans, and its fully faithfulness can be deduced from \cite{BBB+24}*{Lemma 3.7}; we omit the details. 
\end{proof}
...
\begin{example}
    ...
    \]
We thus have that $\Phi_{ij}(\cO_{\sX_i})$ is the pushforward of $\cO_{\Gamma_{ij}}$ under the natural birational morphism $\Gamma_{ij}\to \sX_j$.  We then have $R(q|_{\Gamma_{ij}})_* \cO_{\Gamma_{ij}} = \cO_{\sX_j}$ by \cite{BBB+24}*{Proposition 3.5}.
\end{example}

\begin{example}
    If $i=j$ then $\Gamma_{ij}$ is simply the diagonal $\Delta\subseteq \sX_i\times \sX_i$.  In this case,  $\Phi_{ii}$ is a Fourier--Mukai transform with respect to a resolution of the diagonal.  Thus $\Phi_{ii}$ is equivalent to the identity functor (Proposition~\ref{prop:diagonalidentity}).
\end{example}
...
\]
Let us apply this transform to the Bondal--Thomsen element $\cO_{\mathcal H_3}(-1,0)$.  To perform the computation, we start with the Hanlon-Hicks-Lazarev resolution for $\Gamma_{ij}$.  This has the form
\[
\begin{matrix}
    \cO(0,0,0)\\
    \oplus \\
...
\end{matrix}.
\]
Each line bundle is a box product from the factors.  For instance, $\cO(2,-1,-2)$ means $\cO_{\mathcal H_3}(2,-1) \boxtimes \cO_{\PP(1,1,3)}(-2)$. When we pull back $\cO_{\mathcal H_3}(-1,0)$ and tensor with this complex, we get:
\[
\begin{matrix}
    \cO(-1,0,0)\\
    \oplus \\
...
\end{matrix}.
\]
Call this complex $\cC$. There is a hypercohomology spectral sequence
\begin{equation}
\label{eqn:hyperss}
E^{s,t}_1 = R^tq_*(\cC^s) \Rightarrow R^{s + t}q_*(\cC). 
\end{equation}
As in Construction~\ref{const:monad}, given a coherent sheaf $\cF$ on $\cH_3$ and a locally free sheaf $\cG$ on $\PP(1,1,3)$, the projection formula and base change imply that   $R^tq_*\left(\cF\boxtimes \cG\right) \cong H^t(\cH_3, \F) \otimes_\k \cG$. For all but one summand $\cL \boxtimes \cL'$ of~$\cC$, we have $H^t(\cH_3, \cL) = 0$ for all $t$; the only nonvanishing cohomology that arises is $H^1(\cH_3, \OO(-2, 0)) \cong \k$. We conclude that $E_1^{s, t} = 0$ unless $s = -1$ and $t = 1$, and so $Rq_*(\cC) \cong \OO_{\PP(1,1,3)}(-1)$ (concentrated in cohomological degree 0). 
...
\begin{remark}
    There is also an algebraic method to ``move'' a sheaf from one of the $\sX_i$ to all of the others.
 ...
 For instance, as in Example~\ref{ex:HirzFMTransform}, let us start with $\cO_{\cH_3}(-1,0)$ on $\cH_3$.  We can represent this by a graded $S$-module: for instance $S(-1,0)$ is a natural choice, as $S(-1,0)|_{\cH_3} = \cO_{\cH_3}(-1,0)$.  We can then use $S(-1,0)$ to determine a sheaf on $\PP(1,1,3)$ namely: $S(-1,0)|_{\PP(1,1,3)} = \cO_{\PP(1,1,3)}(-1)$. But one should be cautious, as the outcome of this method depends on the module one uses to represent the initial sheaf.  For instance, we could also have represented $\cO_{\cH_3}(-1,0)$ via a different module.  A rather artificial choice would be to write $M=S(-1,0) \oplus S/(x_0,x_2)$.  Since $(x_0,x_2)$ is a component of the irrelevant ideal for $\cH_3$, we have $(S/(x_0,x_2))|_{\cH_3}=0$ as a sheaf on $\cH_3$.  Thus, $M|_{\cH_3} \cong \cO_{\cH_3}(-1,0)$.  But on $\PP(1,1,3)$, $(x_0,x_2)$ is not a component of the irrelevant ideal; it is the ideal of the singular point  $p=[0:0:1]$.  Thus, we have $M|_{\PP(1,1,3)} = \cO_{\PP(1,1,3)}(-1) \oplus \cO_p$.
...
We get a uniqueness property akin to Corollary~\ref{cor:UniquenessAndFSEC}.  Namely, take a sheaf $\cE$ on a toric variety $X$.  Because $\Theta$ generates $D(X)$, one can represent $\cE$ in terms of the Bondal--Thomsen collection, but this representation will often be fundamentally non-unique.  However, if we want to represent $\cE$ \emph{and} the Fourier--Mukai transforms of $\cE$ to the other $\sX_i$ from the secondary fan, then we can do this in a unique (up to homotopy) way.  This is (very roughly) the statement of Theorem~\ref{thm:homotopycat}.  In short: in some sense, it is simpler to classify sheaves on the whole secondary fan than it is to just classify sheaves on a single toric variety $X$.

So the theorem tells us that while King's Conjecture was false, the underlying idea was very much correct.  The cases where full strong exceptional collections of line bundles exist---projective space, products, projective bundles, etc---are indicative of what happens in the general case.  The correct statement just required a framework that was not yet obvious back then.
\end{remark}
