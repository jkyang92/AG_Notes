...
%\michael{Finish this remark}
%\begin{remark}
%\label{rem:pullpush}
%It is an easy consequence of the projection formula that $\Phi_{i, j} = Rq_* \circ p^*$. (p and q the projections from the graph, so need different notation.) To see this, let $f \co X \to X \times Y$ denote the inclusion of $\Gamma_{ij}$ in $X \times Y$. We have:
%$$
%\Phi_{ij}(\cE) = Rq_*(p^*\cE \otimes_{\cO_{\sX_i\times \sX_j}} f_*\OO_{X}) = R(qf)_*(L(pf)^*\cE \otimes_{\OO_X} \OO_X) = R(qf)_*(\cE)
%$$
%\end{remark}
