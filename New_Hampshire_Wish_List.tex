...
\begin{enumerate}
    \item  Question:  Can we make a Fourier-Mukai transform for line bundles, or locally free sheaves from $\sX_i$ to $\sX_j$, as in the notation of the King Paper?  We will use $S$ to represent the Cox of $\sX_i$ which is also the Cox $\sX_j$.
        \begin{itemize}
            \item  Input the
            \item  Start with a line bundle or locally free sheaf on $\sX_i$, represented as a $S$-module $M$.
            \item  Pull back to the graph of the birational morphism between $\sX_i$ and $\sX_j$.  We need to represent the structure sheaf of the graph as an $S \otimes_k S$-module.  There are various possibilites for doing this. E.g. we could take the cokernel of the HHL resolution; or we could saturate and homgenize the ``naive'' graph;  or we could implement more directly the recipe from the Berkesch et al paper.  Not clear which will be easiest and/or most efficient.   Call the diagonal module $N_{\Delta}$.  Then 
            \[
            (M\otimes_k S) \otimes N_{\Delta}.
            \]
            \item  We apply Zotine's pushforward code to the sheaf corresponding to $(M\otimes_k S) \otimes N_{\Delta}$ with respect to the map from the graph to $\sX_j$.  (Remark:  not totally clear to us what is required for the input of Zotine's HDI (higher direct image) routine in Macaulay2.
        \end{itemize}
    \item  Polishing the notes.
    \item  Write efficient code for just producing the Bondal-Thomsen collection.
    \item  Secondary fan code.  Also go from a multigrading ot the list of admissible irrelevant ideals.
    \item  Vector partition code?  What exists and what it would take to get something that gives a chamber-by-chamber Hilbert quasi-polynomial code for multigraded rings?
\end{enumerate}
