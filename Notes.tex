...
D^b(\PP^n) \ar[r]^-{\overline{\RR}}_-{\simeq} & D^{\on{sing}}(E) \ar[r]_-{\simeq} & K(E). \\
}
\end{equation}
...
With all this in mind, let us now give an alternative formulation of the BGG correspondence for~$\PP^n$. Let $\F$ be a coherent sheaf on $\PP^n$, and let $M$ be a finitely generated graded $S$-module such that $H^0_\m(M) = 0$ and $\widetilde{M} = \F$. Let $G$ be the minimal free resolution of $\RR(M)$, and let $T(\F)$ be the mapping cone of the quasi-isomorphism $G \xra{\simeq} \RR(M)$. 


Let $T \co D^b(\PP^n) \xra{\simeq} K(E)$ denote the functor given by composing the equivalences in the bottom row of this diagram. Our claim is that $T$ is the ``right" geometric version of the BGG correspondence. 
