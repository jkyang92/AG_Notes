...
$$
The only functor here we have not yet described is the rightmost vertical one, which sends an object~$F$ to $\cone(G \xra{\simeq} F)$, where $G$ is the minimal free resolution of~$F$. Let $T \co D^b(\PP^n) \xra{\simeq} K(E)$ denote the functor given by composing the equivalences in the bottom row of this diagram. Our claim is that $T$ is the ``right" geometric version of the BGG correspondence. 
